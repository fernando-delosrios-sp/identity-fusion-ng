...
| **3. Validate new Fusion accounts**                         | Run aggregation; confirm new Fusion accounts are created and data looks correct. **Do not configure Fusion Settings** (matching/review), Attribute Definitions, or any identity scope at this stage, and do **not** configure review settings or select the source owner as a global reviewer. |
...
3. **Leave Fusion and review settings unconfigured**  
   Do not set up **Fusion attribute matches**, **Attribute Definitions**, review forms, or “Automatically correlate if identical?” yet, and do **not** enable **Owner is global reviewer?**. This phase is only about getting the new Fusion accounts into the system and validating data with the minimum possible configuration.
...
## Post-migration reconfiguration and first productive run

After identities have successfully migrated to the new, higher-priority Identity Fusion NG profile:

1. **Re-introduce original sources and settings**
    - Reconfigure **Source Settings → Sources** so that the new Fusion source includes the original managed account sources you used in production (not just the legacy Fusion source).
    - Reapply your previous **deduplication**-related settings (identity scope, Fusion attribute matches, review configuration, etc.) so that the connector behaves like your intended long-term design.
2. **Treat this as your first productive run**
    - The first run after this reconfiguration is effectively your **first productive deduplication run** on the new connector.
    - Only once you reach this step should you configure **Identity Scope**, **Attribute Definitions**, **Fusion Settings**, and (if needed) **Owner is global reviewer?** according to your deduplication and attribute-generation requirements.
