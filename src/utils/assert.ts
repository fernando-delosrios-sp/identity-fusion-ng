...
export function assert<T>(valueOrCondition: T | null | undefined | boolean, message: string): asserts valueOrCondition is T {
 ...
 */
export function softAssert(condition: boolean, message: string, level: 'warn' | 'error' = 'warn'): condition is true {
    if (!condition) {
        const serviceRegistry = ServiceRegistry.getCurrent()
