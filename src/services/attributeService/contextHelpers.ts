...
import type { ParsedAddress } from 'parse-address-string'
...
const parseAddressSync = (addressString: string): ParsedAddress | null => {
    ...
    parseAddressString(addressString, (err, parsed) => {
        error = err
        result = parsed
    })
...
const normalizeFullName = (name: string): string | undefined => {
    ...
    const nameObject = nameNormalizer.parseName(name)
    const firstName = nameObject.firstName ? properCaseName(nameObject.firstName) : ''
    const lastName = nameObject.lastName ? properCaseName(nameObject.lastName) : ''
