...
export const scoreLIG3 = (
    ...
    // Normalize inputs: lowercase, trim, and normalize unicode
    ...
    // Calculate base similarity using modified Levenshtein distance
    const baseScore = calculateLIG3Similarity(s1, s2)

    // Apply token-based bonus for multi-word fields
    const tokenBonus = calculateTokenBonus(s1, s2)

    // Apply positional weighting (common prefix bonus)
...
/**
 * Calculate LIG3 similarity using modified Levenshtein distance with gap penalties
 */
function calculateLIG3Similarity(s1: string, s2: string): number {
    ...
    // Initialize base cases with intelligent gap penalties
    for (let i = 0; i <= len1; i++) {
        matrix[i][0] = i * 0.8 // Reduced penalty for gaps at start/end
    }
    ...
    for (let i = 1; i <= len1; i++) {
        for (let j = 1; j <= len2; j++) {
            ...
            const insertion = matrix[i][j - 1] + 0.9 // Slight penalty for insertion
            const deletion = matrix[i - 1][j] + 0.9 // Slight penalty for deletion
...
/**
 * Calculate bonus score based on matching tokens (words)
 */
function calculateTokenBonus(s1: string, s2: string): number {
    ...
    if (tokens1.length <= 1 && tokens2.length <= 1) {
        return 0 // No token bonus for single words
    }
    ...
    for (const token1 of tokens1) {
        for (let j = 0; j < tokens2.length; j++) {
            if (!used.has(j)) {
                ...
                // Consider tokens matching if they're identical or very similar
...
/**
 * Calculate bonus score for common prefix
 */
function calculatePrefixBonus(s1: string, s2: string): number {
    ...
    // Give significant weight to common prefixes (up to 5 characters)
